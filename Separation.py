    ...
    def inference(self, file_path):
        with torch.no_grad():
            for key, egs in tqdm.tqdm(self.mix):
                ...
                egs_tensor = torch.from_numpy(egs)
                egs=egs_tensor.to(self.device)
...
def main():
    ...
    parser.add_argument(
        '-mix_scp', type=str, default='./inputs/tt_mix.scp', help='Path to mix scp file.')
    parser.add_argument(
    ...
    parser.add_argument(
        '-save_path', type=str, default='./outputs', help='save result path')
